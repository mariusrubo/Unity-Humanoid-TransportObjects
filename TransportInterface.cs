...
using UnityEngine;
using RootMotion; // needed for Interaction Object etc.
using RootMotion.FinalIK; // needed for Interaction Object etc.
...
public class TransportInterface : MonoBehaviour {
    ...
    // Update is called once per frame
        ...
        //if (PickupGuitar) { float Pickedup = CharacterPickUp.PickUpObject(obj2); } // returns value from 0 to 1 indicating when it has picked up the object
    }
    ...
    public int TransportObject(InteractionObject whichObject, GameObject whichGoal, WalkToGoal walktogoal, PickUp pickup, float pickuptime, int cascade)
    {
        ...
        if (cascade == 1) // walk to object
        {
            bool ObjectReached = walktogoal.WalkTo(whichObject.transform, false, 0.4f, 0.3f, 120); // walk towards object to be pickup
        ...
        if (cascade == 3) // walk to goal
        {
            bool GoalReached = walktogoal.WalkTo(whichGoal.transform, false, 0.4f, 0.3f, 120); // walk towards goal
